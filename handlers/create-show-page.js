...
exports.handler = async event => {
  ...
  try {
    ...
    [page, title, `${title} - Video Collection`, `${page}, videos, collection`, title,
      `${title} - Video Collection`, pagePublicPath(page), JSON.stringify(config.presentation),
  ...
  return { statusCode: 201, headers, body: JSON.stringify({ page, title, setup_url: pageSetupUrl(page, setupToken) }) };
};