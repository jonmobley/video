...
Thumbnails: right after an upload the editor is offered the same frame picker as the account dashboard (six frames pulled from the local file, or a custom image ≤ 500 KB). The choice is pushed to Bunny's Set Thumbnail API by `/api/bunny-set-thumbnail`, so the poster lives on the pull zone and no image bytes are stored in Postgres. **Edit Video → Change thumbnail** re-opens the picker later using the five frames Bunny generates during encoding (`thumbnail_1.jpg` … `thumbnail_5.jpg`) plus custom upload.
