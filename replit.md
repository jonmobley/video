...
- **Styling**: Global CSS is managed in `/styles/common.css` (provides CSS reset, base typography, body background/color, focus-visible outlines, @keyframes spin, button/form defaults, source badges, responsive helpers, and utility classes for video placeholders, error states, dialogs, icon pickers, featured content, thumbnails, embeds, and SR-only elements). Gallery-specific responsive/mobile CSS is shared via `/styles/gallery-mobile.css`. Page-specific styles live in external CSS files under `/styles/` (e.g., `login.css`, `upload.css`, `index.css`, `admin.css`, `account.css`, `watch.css`, `disc.css`, `oz-page.css`, `test-page.css`, `vertical-page.css`, `dropbox.css`). All `<style>` blocks have been extracted from HTML into these external CSS files, and all inline `style=` attributes from HTML files and JS template literals have been replaced with CSS classes — the codebase contains zero first-party inline styles.
- **Interactivity**: All page-specific JavaScript lives in external `.js` files under `js/` (e.g., `js/oz-app.js`, `js/login-app.js`, `js/index-hero.js`). Dynamic inline event handlers (onclick/onerror) are replaced with `data-action`/`data-*` attributes and document-level event delegation. JavaScript modules in `/js/` handle client-side logic, Wistia API integration, and various utilities for error display, color manipulation, platform configuration, page configuration, URL management, caching, thumbnail loading, and video platform detection. Shared utility modules imported by oz.html, vertical.html, and test.html include:
    ...
    - `js/shared-admin-dialogs.js` — `createAdminDialogOverlay`, `openIconPickerDialog`, `openDeleteTagDialog`, `openDeleteVideoDialog`, `openFeaturedContentDialog` for shared admin dialog/popup UI. Used by oz.html, vertical.html, and test.html. Each app passes callbacks for app-specific confirm/save behavior.
