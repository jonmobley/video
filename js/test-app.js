        ...
        /**
         * Loads Vimeo thumbnail and duration using oEmbed API
         * @param {string} videoId - The Vimeo video ID
         * @param {string} thumbnailId - The HTML element ID for the thumbnail container
         */
        async function loadVimeoThumbnail(videoId, thumbnailId) {
            try {
                const data = await fetchWithCache(`https://vimeo.com/api/oembed.json?url=https://vimeo.com/${videoId}`);
                    
                    // Load thumbnail
                    if (data.thumbnail_url) {
                        // Get high quality thumbnail
                        const highQualityThumb = data.thumbnail_url.replace(/_\d+\./, '_640.');
                        const imgElement = document.querySelector(`#${thumbnailId} img`);
                        if (imgElement) {
                            imgElement.src = highQualityThumb;
                    ...
                    // Load duration
                    if (data.duration) {
                        const durationElement = document.querySelector(`#${thumbnailId} .thumbnail-duration`);
                        if (durationElement) {
                ...
                console.error('Error loading Vimeo data:', error);
            }
        }
        
        // Adjust video container dimensions based on aspect ratio
        function adjustVideoContainer(aspectRatio) {
            const videoContainerElement = document.querySelector('.video-container');
            const isMobile = window.innerWidth <= 768;
            
            if (isMobile) {
                // On mobile, always use full width and adjust height
                videoContainerElement.style.width = '100%';
                videoContainerElement.style.height = 'auto';
                videoContainerElement.style.aspectRatio = `${aspectRatio}`;
                return;
            }
            
            // Original maximum container dimensions (16:9 at 1000px width)
            const maxContainerWidth = 1000;
            const maxContainerHeight = maxContainerWidth / (16/9); // ~562.5px
            
            let width, height;
            
            // Calculate dimensions that fit within the max container while maintaining aspect ratio
            if (aspectRatio > (maxContainerWidth / maxContainerHeight)) {
                // Video is wider than container ratio - fit to width
                width = maxContainerWidth;
                height = width / aspectRatio;
            } else {
                // Video is taller than container ratio - fit to height
                height = maxContainerHeight;
                width = height * aspectRatio;
            }
            
            // Apply dimensions
            videoContainerElement.style.width = `${width}px`;
            videoContainerElement.style.height = `${height}px`;
            videoContainerElement.style.maxWidth = `${maxContainerWidth}px`;
            videoContainerElement.style.maxHeight = `${maxContainerHeight}px`;
            videoContainerElement.style.aspectRatio = `${aspectRatio}`;
        }
        
        // Reset video container to default dimensions
        function resetVideoContainer() {
            const videoContainerElement = document.querySelector('.video-container');
            videoContainerElement.style.width = '';
            videoContainerElement.style.height = '';
            videoContainerElement.style.maxWidth = '';
            videoContainerElement.style.maxHeight = '';
            videoContainerElement.style.aspectRatio = '';
        }
        ...
        /**
         * Sets the active video state and updates UI accordingly
         * @param {string|null} wistiaId - The ID of the video to set as active, or null to clear
         */
        ...
        // Function to update categories dropdown with icons - REMOVED as dropdown is no longer used
        // function updateCategoriesDropdown(categoriesData) {
        //     const dropdown = document.getElementById('categoriesDropdown');
        //     if (!dropdown) return;
        //     
        //     // Clear existing options except the default ones
        //     const defaultOptions = Array.from(dropdown.options).filter(option => 
        //         option.value === 'the-merry-land-of-oz' || option.value === ''
        //     );
        //     
        //     dropdown.innerHTML = '';
        //     
        ...
        //     
        //     // Add category-based options if needed in the future
        //     if (categoriesData && categoriesData.length > 0) {
        //         categoriesData.forEach(category => {
        //             if (category.showInDropdown) {
        ...
        /**
         * Stops the current video and closes the player container
         * Handles cleanup for all video platforms (Wistia, Vimeo, YouTube)
         */
        function stopVideoAndClosePlayer() {
            console.log('Stopping video and closing player');
            
            // Stop the current video based on platform
            const activeItem = document.querySelector('.video-item.active');
            if (activeItem) {
                const platform = activeItem.dataset.platform || 'wistia';
                
                if (platform === 'wistia' && window.Wistia && currentlyPlayingVideoId) {
                ...
                // For iframe-based players (Vimeo, YouTube), removing the iframe will stop playback
            }
            
            // Hide the video container with smooth animation
            const videoContainerElement = document.querySelector('.video-container');
            videoContainerElement.classList.remove('active');
            videoContainerElement.removeAttribute('data-platform');
        ...
        /**
         * Resets the page to default state - closes video, clears search, shows all videos
         * Called when clicking the page title
         */
        function resetToDefault() {
            ...
            // Dropdown removed - no longer need to reset it
            // const dropdown = document.getElementById('categoriesDropdown');
            // if (dropdown) {
            //     dropdown.value = 'all-songs';
            // }
            
            // Reset tag selection to "All"
            ...
            // Show all videos (in case any were filtered)
        ...
        // Generic video loading function
        function loadVideo(videoUrl, title, platform = null) {
            ...
            // Route to appropriate platform loader
            switch(platform) {
                case 'wistia':
                    loadWistiaVideo(videoId, title);
                    break;
                case 'vimeo':
                    loadVimeoVideo(videoId, title);
                    break;
                case 'youtube':
                    loadYouTubeVideo(videoId, title);
                    break;
                case 'dropbox':
                    loadDropboxVideo(videoUrl, title);
                    break;
                default:
        ...
        // Load Vimeo video
        async function loadVimeoVideo(vimeoId, title) {
            console.log('Loading Vimeo video:', vimeoId, title);
            
            // Show video container
            const videoContainerElement = document.querySelector('.video-container');
            videoContainerElement.classList.add('active');
            videoContainerElement.setAttribute('data-platform', 'vimeo');
            
            // Get video dimensions from oEmbed API
            try {
                const data = await fetchWithCache(`https://vimeo.com/api/oembed.json?url=https://vimeo.com/${vimeoId}`);
                const aspectRatio = data.width / data.height;
                
                // Adjust container dimensions based on aspect ratio
                adjustVideoContainer(aspectRatio);
            } catch (error) {
                console.error('Error fetching Vimeo video dimensions:', error);
                // Default to 16:9 if we can't get dimensions
                adjustVideoContainer(16/9);
            }
            ...
            // Hide title overlay for Vimeo (they have their own)
            const titleElement = document.getElementById('current-video-title');
            if (titleElement) {
            ...
            currentlyPlayingVideoId = vimeoId;
        }
        
        // Load YouTube video
        function loadYouTubeVideo(youtubeId, title) {
            console.log('Loading YouTube video:', youtubeId, title);
            
            // Show video container
            const videoContainerElement = document.querySelector('.video-container');
            videoContainerElement.classList.add('active');
            videoContainerElement.setAttribute('data-platform', 'youtube');
            
            // YouTube doesn't provide dimensions via API without authentication
            // Default to 16:9 for most YouTube videos
            adjustVideoContainer(16/9);
            ...
            videoContainer.innerHTML = `
                <iframe 
                    src="https://www.youtube.com/embed/${youtubeId}?autoplay=1&rel=0&modestbranding=1&controls=1&disablekb=0&fs=1&iv_load_policy=3&loop=0&showinfo=0&playsinline=1"
                    style="position:absolute;top:0;left:0;width:100%;height:100%;"
                    frameborder="0"
            ...
            // Hide title overlay for YouTube (they have their own)
            const titleElement = document.getElementById('current-video-title');
            if (titleElement) {
                titleElement.style.display = 'none';
            }
            ...
            currentlyPlayingVideoId = youtubeId;
        }
        
        // Load Wistia video (original function)
        function loadWistiaVideo(wistiaId, title) {
            ...
            const videoContainerElement = document.querySelector('.video-container');
            videoContainerElement.classList.add('active');
            videoContainerElement.setAttribute('data-platform', 'wistia');
            
            // Reset to standard 16:9 for Wistia
            resetVideoContainer();
            ...
            // Update video title and ensure it's visible
            const titleElement = document.getElementById('current-video-title');
            if (titleElement) {
                titleElement.textContent = title;
                titleElement.style.display = ''; // Reset display to show it
            }
            ...
            // Ensure Wistia script is loaded
        ...
        function handleOrientationChange() {
            if (currentWistiaVideo && window.innerWidth <= 768) {
        ...
        /**
         * Loads videos from the server via Netlify Functions
         * Fetches videos for the current page (disc) and renders the grid
         */
        async function loadVideosFromServer() {
            try {
                ...
                const errorType = error.message && error.message.startsWith('HTTP') ? 'server' : 'network';
                showError(errorType);
            }
        }

        // ====================================================================
        // VIDEO GRID RENDERING AND DISPLAY
        // ====================================================================
        ...
        function renderVideoGrid(videos) {
            ...
            const videoHTML = videosToDisplay.map((video, index) => {
                ...
                const videoUrl = video.video_url || `https://videosharepro.wistia.com/medias/${video.wistiaId}`;
                ...
                const html = `
                    <div class="video-item" data-category="${video.category}" data-tags="${tagsString}" data-title="${video.title}" data-wistia="${video.wistiaId}" data-video-url="${videoUrl}" data-platform="${platform}">
                        ...
                        <div class="thumbnail" id="thumb-${video.wistiaId}">
                            <img src="${getThumbnailUrl(videoUrl, platform, extractVideoId(videoUrl, platform) || video.wistiaId)}" alt="${video.title}" style="width: 100%; height: 100%; object-fit: cover; position: relative; z-index: 1; background: #2a2a2a;" data-img-hide-on-error="true">
            ...
            // Load durations for displayed videos only
            videosToDisplay.forEach(video => {
                loadVideoDuration(video.wistiaId, testCacheOptions);
            });
            
            // Load Vimeo thumbnails asynchronously
            videosToDisplay.forEach(video => {
                const platform = video.platform || detectVideoPlatform(video.video_url) || 'wistia';
                if (platform === 'vimeo') {
                    const videoId = extractVideoId(video.video_url, 'vimeo');
                    if (videoId) {
                        loadVimeoThumbnail(videoId, `thumb-${video.wistiaId}`);
                    }
                }
            });
        ...
        function attachVideoListeners() {
            ...
            videoItems.forEach(item => {
                item.addEventListener('click', () => {
                    ...
                    const videoUrl = item.dataset.videoUrl || `https://videosharepro.wistia.com/medias/${wistiaId}`;
                    const platform = item.dataset.platform || 'wistia';
                    ...
                    if (currentlyPlayingVideoId === wistiaId) {
                        ...
                        if (platform === 'wistia') {
                            ...
                            loadVideo(videoUrl, title, platform);
                        }
        ...
        // Debouncing for category clicks
        let categoryClickTimeout = null;
        let isFilteringInProgress = false;

        // Tag filtering - consolidated to use single filtering mechanism
        ...
        function loadPageConfigDisc() {
            return loadPageConfig('disc', {
                fetchFn: fetchWithCache
            });
        }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (config.accent_color) {
                        document.getElementById('adminAccentColor').value = config.accent_color;
                        document.getElementById('adminAccentColorText').value = config.accent_color;
                    }
                }
            } catch (error) {
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            // Load page config first to set colors
            await loadPageConfigDisc();
            loadVideosFromServer();
            attachTagListeners();
            
            // Page Settings form handlers
            document.getElementById('pageSettingsForm').addEventListener('submit', savePageSettings);
            document.getElementById('cancelPageSettings').addEventListener('click', function() {
                document.getElementById('pageSettingsOverlay').style.display = 'none';
                document.getElementById('pageSettingsError').style.display = 'none';
            });
        ...
        function enterEditMode() {
            ...
            // Set featured button listener
            document.getElementById('adminSetFeaturedBtn').addEventListener('click', openSetFeaturedDialog);
            
            // Page settings button listener
            document.getElementById('adminPageSettingsBtn').addEventListener('click', openPageSettingsModal);
        ...
        function addCategoryEditListeners() {
            document.querySelectorAll('.tag:not(.add-category-btn)').forEach(tag => {
                ...
                tag.addEventListener('click', function(e) {
                    ...
                    // X button is in top-right corner (last 18px of width and height)
                    if (clickX > rect.width - 18 && clickY < 18) {
                        e.stopPropagation();
        ...
        function deleteCategoryWithReassignment(categoryElement) {
            ...
            if (videosInCategory.length === 0) {
                ...
                if (confirm(`Delete tag "${categoryName}"?`)) {
                    categoryElement.remove();
                    markUnsavedChanges();
                    console.log('Tag deleted:', categoryName);
                }
                return;
            }
            ...
            dialog.querySelector('#confirmDelete').addEventListener('click', function() {
                ...
                if (!newCategoryId) {
                    alert('Please select a tag for reassignment.');
                    return;
                }
        ...
        function addEditListeners() {
            ...
            document.getElementById('addCategoryBtn').addEventListener('click', createNewCategory);
        ...
        /**
         * Moves a video in the grid using arrow controls
         * @param {Event} event - The click event
         * @param {string} wistiaId - The ID of the video to move
        ...
        /**
         * Gets the number of columns in the video grid based on viewport width
        ...
        async function saveAllChanges() {
            ...
            try {
                ...
                showErrorMessage('Unable to save changes. Please check your connection and try again.');
        ...
        /**
         * Opens the edit video popup with current video details
         * @param {string} wistiaId - The ID of the video to edit
         * @param {string} title - Current video title
         * @param {string} category - Current video category
         */
        ...
        function populateEditCategoryDropdown() {
            ...
            const categories = Array.from(document.querySelectorAll('.tag:not(.add-category-btn)')).map(tag => ({
                id: tag.dataset.category,
                name: tag.textContent
            }));
        ...
        document.getElementById('editVideoForm').addEventListener('submit', function(e) {
            ...
            if (!newTitle || !newCategory) {
                document.getElementById('editVideoError').textContent = 'Please fill in all fields';
            ...
            if (videoItem) {
                ...
                if (tagsContainer) {
                    tagsContainer.innerHTML = `<span class="item-tag-pill">${newCategory.charAt(0).toUpperCase() + newCategory.slice(1)}</span>`;
                }
                ...
                editBtn.dataset.videoTitle = newTitle;
                editBtn.dataset.videoCategory = newCategory;
            }
            
            console.log('Video updated:', { id: currentEditingVideoId, title: newTitle, category: newCategory });
        ...
        document.getElementById('deleteVideoBtn').addEventListener('click', function() {
            if (confirm('Are you sure you want to delete this video? This action cannot be undone.')) {
                ...
                if (videoItem) {
                    videoItem.remove();
                    console.log('Video deleted:', currentEditingVideoId);
        ...
        /**
         * Opens the add video popup and initializes the form
         */
        ...
        function populateAddCategoryDropdown() {
            console.log('📋 DEBUG: === POPULATING CATEGORY DROPDOWN AND TAGS ===');
            ...
            console.log('📋 DEBUG: Setting default option');
            select.innerHTML = '<option value="">Select Category</option>';
            ...
            console.log('📋 DEBUG: Adding categories to dropdown');
            categories.forEach(cat => {
                if (cat.id !== 'all') {
                    console.log('📋 DEBUG: Adding category:', cat);
                    const option = document.createElement('option');
                    option.value = cat.id;
                    option.textContent = cat.name;
                    select.appendChild(option);
            ...
            console.log('📋 DEBUG: Category dropdown populated with', select.children.length, 'options');
            
            // Also populate tag selection
            ...
            categories.forEach(cat => {
                if (cat.id !== 'all') {
                    ...
                    tagSelector.dataset.tagId = cat.id;
                    tagSelector.textContent = cat.name;
        ...
        // Video Link Input Handler (supports multiple platforms)
        document.getElementById('wistiaLink').addEventListener('input', async function(e) {
            ...
            if (platform && videoId) {
                ...
                try {
                    ...
                    switch(platform) {
                        case 'wistia':
                            videoData = await fetchWistiaVideoData(videoId);
                            break;
                            
                        case 'vimeo':
                            // For Vimeo, we'd need to use their oEmbed API
                            // For now, just use manual entry
                        ...
                        case 'youtube':
                            // For YouTube, we could use their API but it requires an API key
                    ...
                    console.log(`Loaded ${platform} data:`, { id: videoId, ...videoData });
                } catch (error) {
                    console.error('Error loading video data:', error);
                    titleInput.placeholder = 'Enter video title manually...';
                    titleInput.disabled = false;
                }
            } else {
                titleInput.value = '';
                titleInput.placeholder = 'Invalid video URL';
            }
        });
        ...
        document.getElementById('addVideoForm').addEventListener('submit', async function(e) {
            ...
            const videoUrl = document.getElementById('wistiaLink').value.trim();
            ...
            if (!videoUrl || !title || !category) {
                document.getElementById('addVideoError').textContent = 'Please fill in all fields';
            ...
            if (!platform || !videoId) {
                document.getElementById('addVideoError').textContent = 'Invalid video URL';
                document.getElementById('addVideoError').style.display = 'block';
                return;
            }
            ...
            if (!validateVideoId(videoId, platform)) {
                document.getElementById('addVideoError').textContent = 'Invalid video ID format';
                document.getElementById('addVideoError').style.display = 'block';
                return;
            }
            
            // For non-Wistia platforms, use the video ID as wistiaId for now
            const wistiaId = platform === 'wistia' ? videoId : `${platform}_${videoId}`;
            
            // Check if video already exists
            if (document.querySelector(`[data-wistia="${wistiaId}"]`)) {
                document.getElementById('addVideoError').textContent = 'This video already exists';
                document.getElementById('addVideoError').style.display = 'block';
                return;
            }
            ...
            console.log('Video added:', { id: wistiaId, title, category, tags: selectedTags });
        });
        ...
        async function addVideoToGrid(video) {
            ...
            loadWistiaThumbnail(video.wistiaId, testCacheOptions);
            loadVideoDuration(video.wistiaId, testCacheOptions);
        ...
        /**
         * Validates video ID format for different platforms
         * @param {string} videoId - The video ID to validate
         * @param {string} platform - The platform (wistia, vimeo, youtube)
        ...
        function validateVideoId(videoId, platform) {
            ...
            return patterns[platform]?.test(videoId) || false;
        }
        ...
        /**
         * Opens the page settings modal and loads current settings
         */
        async function openPageSettingsModal() {
            try {
                ...
                if (response.ok) {
                    ...
                    document.getElementById('accentColor').value = config.accent_color || '#ff6b6b';
                    document.getElementById('accentColorText').value = config.accent_color || '#ff6b6b';
                    document.getElementById('metaDescription').value = config.meta_description || '';
                    document.getElementById('metaKeywords').value = config.meta_keywords || '';
                    document.getElementById('canonicalUrl').value = config.canonical_url || '';
                    ...
                    document.getElementById('twitterTitle').value = config.twitter_title || '';
                    document.getElementById('twitterDescription').value = config.twitter_description || '';
                }
            } catch (error) {
                console.error('Failed to load page settings:', error);
                showErrorMessage('Unable to load page settings. Please try again.');
            }
            
            // Show the modal
            document.getElementById('pageSettingsOverlay').style.display = 'flex';
        }
        
        /**
         * Saves page settings to the server
         */
        async function savePageSettings(e) {
            ...
            const settings = {
                page: 'disc',
                accent_color: document.getElementById('accentColorText').value.trim() || document.getElementById('accentColor').value,
                ...
                meta_keywords: document.getElementById('metaKeywords').value.trim(),
                canonical_url: sanitizeUrl(document.getElementById('canonicalUrl').value.trim()),
                og_title: document.getElementById('ogTitle').value.trim(),
                og_description: document.getElementById('ogDescription').value.trim(),
                og_image_url: sanitizeUrl(document.getElementById('ogImageUrl').value.trim()),
                twitter_title: document.getElementById('twitterTitle').value.trim(),
                twitter_description: document.getElementById('twitterDescription').value.trim()
            };
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    showSuccessMessage('Page settings saved successfully!');
                } else {
                    throw new Error('Failed to save settings');
                }
            } catch (error) {
                console.error('Error saving page settings:', error);
                showErrorMessage('Unable to save settings. Please check your connection and try again.');
            }
        }
        
        // Function to activate a category and filter videos
        function activateCategory(categoryName) {
            ...
            try {
                ...
                categoryName = categoryName ? categoryName.toLowerCase() : 'all';
                
                // Don't re-filter if already on this category
                if (currentActiveCategory === categoryName) {
                    return;
                }
                
                console.log('Activating category:', categoryName);
                
                // Update active category state
                currentActiveCategory = categoryName;
                
                // Remove active class from all navigation tags
                document.querySelectorAll('.tag').forEach(tag => {
                    tag.classList.remove('active');
                });
                
                // Find and activate the corresponding navigation tag
                const navTag = document.querySelector(`[data-category="${categoryName}"]`);
                if (navTag) {
                    navTag.classList.add('active');
                } else {
                    // If category not found in nav, activate "All"
                ...
                // Filter videos based on tags - use consistent method
                const videoItems = document.querySelectorAll('.video-item');
                videoItems.forEach(item => {
                    // Clear any existing display styles to ensure consistency
                    item.style.display = '';
                    ...
                    if (categoryName === 'all') {
                        ...
                        const videoTags = item.dataset.tags ? item.dataset.tags.split(',') : [item.dataset.category];
                        const hasTag = videoTags.includes(categoryName);
                ...
                console.log('Category filtering completed:', categoryName);
            } finally {
        ...
        // Initialize category navigation on page load
        document.addEventListener('DOMContentLoaded', function() {
            // Add click handlers to navigation tags
        ...
        document.addEventListener('error', function(e) {
            ...
            if (e.target.hasAttribute('data-thumb-fallback')) {
                ...
                if (typeof loadWistiaThumbnail === 'function') loadWistiaThumbnail(wId, testCacheOptions);
            }
        }, true);
