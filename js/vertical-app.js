        ...
        // Track which videos are vertical format (9:16)
        let verticalVideoIds = new Set();

        const verticalCacheOptions = {
        ...
        // Platform manager for multi-platform support
        ...
        // Function to detect and handle vertical videos
        function checkVideoAspectRatio(wistiaId) {
            // Use Wistia API to get video dimensions
            if (window.Wistia) {
                window.Wistia.api(`wistia_${wistiaId}`, function(video) {
                    if (video) {
                        ...
                        if (width && height) {
                            ...
                            console.log(`Video ${wistiaId} dimensions: ${width}x${height}, aspect ratio: ${aspectRatio}`);
                            
                            // If aspect ratio is less than 1, it's vertical (height > width)
                            if (aspectRatio < 1) {
                                verticalVideoIds.add(wistiaId);
                                applyVerticalVideoStyles(wistiaId);
                            }
                        }
                    }
                });
        ...
        function applyVerticalVideoStyles(wistiaId) {
            // Apply to video container when playing
            const videoContainer = document.getElementById('wistia-player');
            if (currentlyPlayingVideoId === wistiaId && videoContainer) {
                videoContainer.classList.add('vertical');
            }
            
            // Apply to thumbnail
            const thumbnail = document.querySelector(`[data-wistia="${wistiaId}"] .thumbnail`);
            if (thumbnail) {
                thumbnail.classList.add('vertical');
            }
            
            // Add mixed-aspect class to grid
            const videoGrid = document.getElementById('videoGrid');
            if (videoGrid && verticalVideoIds.size > 0) {
                videoGrid.classList.add('mixed-aspect');
            }
        }
        ...
        function loadVideo(videoUrl, title, platform = null, videoId = null) {
            // If platform not provided, detect it
            if (!platform) {
                platform = detectVideoPlatform(videoUrl);
            }
            
            // If videoId not provided, extract it
            if (!videoId) {
            ...
            switch(platform) {
                case 'wistia':
                    ...
                    break;
                case 'vimeo':
                    loadVimeoVideo(videoId, title, videoUrl);
                    break;
                case 'youtube':
                    loadYouTubeVideo(videoId, title);
                    break;
                case 'dropbox':
                    loadDropboxVideo(videoUrl, title, videoId);
                    break;
                default:
        ...
        // Handle thumbnail loading errors for different platforms
        function handleThumbnailError(videoId, platform) {
            const thumbnailElement = document.getElementById(`thumb-${videoId}`);
            ...
            thumbnailElement.innerHTML = `
                ...
                <div class="thumbnail-duration" id="thumb-duration-${videoId}">--:--</div>
                <div class="thumbnail-play-button"></div>
                <div class="featured-controls">
                    <button class="featured-btn" data-action="set-featured" data-wistia-id="${videoId}">Feature</button>
                </div>
                <div class="video-edit-overlay">
                    <button class="video-edit-btn" data-action="edit-video" data-wistia-id="${videoId}" data-video-title="" data-video-category="">Edit</button>
                </div>
        ...
        function loadWistiaVideo(wistiaId, title) {
            ...
            _wq.push({
                ...
                onReady: function(video) {
                    ...
                    // Check video aspect ratio for vertical video detection
                    checkVideoAspectRatio(wistiaId);
        ...
        // Load Vimeo video
        function loadVimeoVideo(vimeoId, title, videoUrl) {
            ...
            // Clear any existing content and load new video
            videoContainer.innerHTML = `
                <iframe src="https://player.vimeo.com/video/${vimeoId}?autoplay=1&title=0&byline=0&portrait=0&badge=0&autopause=0&player_id=0&app_id=58479" 
                        frameborder="0" 
        ...
        function loadYouTubeVideo(youtubeId, title) {
            ...
            // Clear any existing content and load new video
            videoContainer.innerHTML = `
                <iframe src="https://www.youtube.com/embed/${youtubeId}?autoplay=1&rel=0&modestbranding=1&controls=1&disablekb=0&fs=1&iv_load_policy=3&loop=0&playsinline=1" 
                        frameborder="0" 
        ...
        // Load Dropbox video
        function loadDropboxVideo(videoUrl, title, videoId) {
            console.log('Loading Dropbox video:', videoUrl, title);
            
            // Show video container
            const videoContainerElement = document.querySelector('.video-container');
            videoContainerElement.classList.add('active');
            videoContainerElement.setAttribute('data-platform', 'dropbox');
            
            // Convert Dropbox URL to direct streaming URL
            let directUrl = videoUrl;
            try {
                const converted = DropboxURLHandler.convertToDirectUrl(videoUrl);
        ...
        function renderVideoGrid(videos) {
            ...
            const videoHTML = videosToDisplay.map((video, index) => {
                ...
                // Determine platform and get appropriate thumbnail
                const platform = video.platform || 'wistia';
                const videoId = video.wistiaId; // This is used as the unique ID regardless of platform
                const thumbnailUrl = getThumbnailUrl(video.video_url || '', platform, videoId);
                
                const html = `
                    <div class="video-item" data-category="${safeCategory}" data-tags="${tagsString}" data-title="${safeTitle}" data-wistia="${safeWistiaId}" data-platform="${escapeHtml(platform)}" data-video-url="${escapeHtml(video.video_url || '')}">
                        ...
                        <div class="thumbnail" id="thumb-${safeWistiaId}">
                            ${thumbnailUrl ? 
                                `<img src="${escapeHtml(thumbnailUrl)}" alt="${safeTitle}" class="thumb-img-cover" data-thumb-fallback="${safeWistiaId}" data-thumb-platform="${escapeHtml(platform)}">` :
            ...
            videosToDisplay.forEach(video => {
                loadVideoDuration(video.wistiaId, verticalCacheOptions);
                // Check for vertical video format after a delay to ensure Wistia is ready
                setTimeout(() => checkVideoAspectRatio(video.wistiaId), 1000);
            });
        ...
        function loadPageConfigVertical() {
            return loadPageConfig('vertical');
        }
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            await loadPageConfigVertical();
            // Load category preferences (including custom "All" label)
        ...
        function populateAddCategoryDropdown() {
            ...
            // Get unique categories from loaded video data
            ...
            console.log('📋 DEBUG: Found categories from video data:', Array.from(uniqueCategories));
            ...
            console.log('📋 DEBUG: Category dropdown populated with', select.children.length - 1, 'categories');
            
            // Also populate tag selection (using tags, not categories)
            const tagContainer = document.getElementById('tagSelectionContainer');
            tagContainer.innerHTML = '';
            
            const tags = Array.from(document.querySelectorAll('.tag:not(.add-category-btn)')).map(tag => ({
                id: tag.dataset.category,
                name: tag.textContent
            }));
        ...
        // Multi-platform Video Link Input Handler
        document.getElementById('videoLink').addEventListener('input', async function(e) {
            ...
            if (platform) {
                titleInput.placeholder = `Loading ${platform} title...`;
                titleInput.disabled = true;
                
                try {
                    let videoData = { title: 'Untitled Video' };
                    
                    if (platform === 'wistia') {
                        const wistiaId = extractVideoId(url, 'wistia');
                        if (wistiaId) {
                            videoData = await fetchWistiaVideoData(wistiaId);
                        }
                    } else if (platform === 'vimeo') {
                        const vimeoId = extractVideoId(url, 'vimeo');
                        if (vimeoId) {
                            videoData = await fetchVimeoVideoData(vimeoId);
                        }
                    } else if (platform === 'youtube') {
                        const youtubeId = extractVideoId(url, 'youtube');
                        if (youtubeId) {
                            videoData = { title: `YouTube Video ${youtubeId}` };
                        }
                    } else if (platform === 'dropbox') {
                        try {
                            const converted = DropboxURLHandler.convertToDirectUrl(url);
                            videoData = { title: converted.title };
                        } catch (error) {
                            videoData = { title: 'Dropbox Video' };
                        }
                    }
                    ...
                    console.log(`Loaded ${platform} data:`, videoData);
                } catch (error) {
                    console.error(`Error loading ${platform} data:`, error);
                    titleInput.value = `${platformInfo({platform}).label} Video`;
                    titleInput.disabled = false;
                    titleInput.placeholder = 'Enter video title...';
                }
            } else {
                titleInput.value = '';
                titleInput.placeholder = 'Unsupported video platform';
                titleInput.disabled = false;
            }
        });
        ...
        document.getElementById('addVideoForm').addEventListener('submit', async function(e) {
            ...
            if (!platform) {
                document.getElementById('addVideoError').textContent = 'Unsupported video platform';
                document.getElementById('addVideoError').style.display = 'block';
                return;
            }
            ...
            if (!videoId) {
                document.getElementById('addVideoError').textContent = `Invalid ${platform} URL`;
                document.getElementById('addVideoError').style.display = 'block';
                return;
            }
            
            // Generate unique wistiaId for non-Wistia platforms
            const uniqueId = platform === 'wistia' ? videoId : `${platform}_${videoId}`;
            
            // Check if video already exists
            if (document.querySelector(`[data-wistia="${uniqueId}"]`)) {
            ...
            console.log('Video added:', { id: uniqueId, title, category, tags: selectedTags, platform, url: videoUrl });
        });
        ...
        async function addVideoToGrid(video) {
            ...
            loadWistiaThumbnail(video.wistiaId, verticalCacheOptions);
            loadVideoDuration(video.wistiaId, verticalCacheOptions);
        ...
        function addVideoEventListeners(videoElement) {
            ...
            videoElement.addEventListener('click', () => {
                ...
                const wistiaId = videoElement.dataset.wistia;
                const title = videoElement.dataset.title;
                const platform = videoElement.dataset.platform || 'wistia';
                ...
                if (currentlyPlayingVideoId === wistiaId) {
                    ...
                    // Load the new video using generic function
                    if (platform === 'wistia') {
                        loadWistiaVideo(wistiaId, title);
                    } else {
                        loadVideo(videoUrl, title, platform, wistiaId);
                    }
                    window.scrollTo({ top: 0, behavior: 'smooth' });
                }
            });
        ...
        function populateManageCategoriesPopup() {
            ...
            // Get current categories from loaded video data
            const uniqueCategories = new Set();
            if (window.loadedVideos && Array.isArray(window.loadedVideos)) {
                window.loadedVideos.forEach(video => {
                    if (video.category && video.category !== 'all') {
            ...
            // Add category items
            Array.from(uniqueCategories).forEach((categoryId, index) => {
                addCategoryItemToList(categoryId, categoryId === 'the-merry-land-of-oz' ? 'The Merry Land of Oz' : 
                    categoryId.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '), index);
            });
        ...
        function addCategoryItemToList(id, name, order) {
            const categoriesList = document.getElementById('categoriesList');
        ...
        function saveCategories() {
            ...
            try {
                ...
                // Update the "All" tag text
                updateAllTagLabel();
                
                // Here you would typically save to the server
                // For now, we'll just update the local state and close the popup
        ...
        document.addEventListener('error', function(e) {
            ...
            if (e.target.hasAttribute('data-thumb-fallback')) {
                ...
                var platform = e.target.dataset.thumbPlatform;
                if (platform && typeof handleThumbnailError === 'function') {
                    handleThumbnailError(wId, platform);
                } else if (typeof loadWistiaThumbnail === 'function') {
                    loadWistiaThumbnail(wId, verticalCacheOptions);
                }
            }
        }, true);
